...
export default (db) => {
  ...
  return {
    ...
    beginSlackAssociation(slackUserId) {
      ...
      return slack.conversations.open({ users: slackUserId })
        .then((r) => {
          associationLink.dmChannelId = r.channel.id;
          const serverConfig = config.get('server');
          const authUrl = new URL(config.get('routes.associationPath'), `${serverConfig.protocol}://${serverConfig.host}:${serverConfig.port}`);
          authUrl.searchParams.set('ref', associationLink.ref);
          const slackMessage = slack.chat.postMessage({
    ...
    completeSlackAssociation(userId, associationRef) {
      return dataStoreOperation(linkStore, 'get', associationRef)
        ...
        .then(associationLink => Promise.all([
          ...
          slack.chat.postMessage({ channel: associationLink.dmChannelId, text: 'Well, it\'s nice to meet you! Thanks for completing authentication.' }),
        ]))
